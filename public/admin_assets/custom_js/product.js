...
$("#save_forms").on("submit",function(e){
    ...
    $.ajaxSetup({
     ...
     fd = new FormData(this);
      $.ajax({ 
          type:"post",
          url:siteUrl + "/project-management/save",  
          data:fd,
          processData: false, 
          contentType: false, 
          success:function(res){
              if(res.status_code == 200){
                ...
                //   reset();
                  window.location.href = siteUrl+ "/project-management/view";
  ...
  function showDatatable(){
      ...
      var table = $('#cat_datatable').DataTable({
          processing: true,
          serverSide: true,
          ajax: siteUrl+"/project-management/show",
          columns: [ 
              {data: 'DT_RowIndex'},
              {data: 'name', name: 'name'},
  ...
  function deletes(id = ''){
      if (confirm("Are you sure!")) {
          $.ajax({
              url:siteUrl+"/project-management/delete",
              type:'GET',
              data:{id:id},
  ...
  function edit(id = '') {
      ...
      $.ajax({
          url: siteUrl +"/project-management/edit",
          data: { id: id },
          type: 'GET',
          dataType: "json",
          success: function (res) {
  ...
  function status(id = "", status = "") {
      $.ajax({
          url: siteUrl +"/project-management/status",
  ...
  function view(id = '') {
      // 
      $.ajax({
          url: siteUrl +"/project-management/price_calc",
          data: { id: id},
          type: "get",
          dataType: "json",
          success: function (response) {
  ...
  function sub_cats(id){
     ...
     $.ajax({    
           url:siteUrl +'/project-management/sub_cat',      
           type:'get',      
           data:{'cat_id':id},      
           success:function(response){ 
  ...
  $(".add_field_button").click(function(e){ 
      e.preventDefault();
      let catHtML = $('#categories').html()
      var html = '<div class="col-lg-12"><h3>Image Categories</h3></div><div class="form-group col-lg-4"> <label for="exampleInputEmail1">Category</label> <select class="form-control" name="categories['+i+']">'+catHtML+'</select> </div> <div class="form-group col-lg-4"> <label for="exampleInputEmail1">Value</label> <input type="file" class="form-control" name="image'+i+'[]" multiple accept="image/*" placeholder="Enter value .." value=""> </div> <div class="col-md-4 mt-4 ap_data"><button class="remove_field btn btn-danger btn-xs">-</button></div>';
      i++;
  ...
  $('#import_product').on('submit',function(e){ 
      ...
      console.log(allData);
        ...
        $.ajax({    
           url:siteUrl+'/project-management/import_product',      
           type:'post',      
           data:allData,      
           dataType:'json',
           processData: false, 
