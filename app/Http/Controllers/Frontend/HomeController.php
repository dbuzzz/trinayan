...
use App\Models\{Brand,Category,SubCategory,Product,Contact,HomePage,NewsLetter,ContactUsPage,GenralPage,Address,User,EclinicPage,DoctorCategory,DoctorInfo,DoctorReview,Coupon,CouponUsed,ProductVariant,Blog,BlogCategory,Wishlist,Order,OrderDetail,HomeBanner,BrandPartner,Testimonial,Taxation,Project};
use Validator;
use Auth;
use Str;
use Schema;
...
class HomeController extends Controller
{
    ...
    {
        $data['project'] = Project::where(['is_deleted'=>0,'active_status'=>1,'category'=>'Completed'])->get();
        // dd($data['project']);
      $data['title'] = 'completed';
      return view('frontend.completed',$data);
    }//end of method
    public function underConstruction()
    {
    $data['project'] = Project::where(['is_deleted'=>0,'active_status'=>1,'category'=>'Under Construction'])->get();
      $data['title'] = 'underConstruction';
      return view('frontend.underConstruction',$data);
    }//end of method
    public function upcoming()
    {
        $data['project'] = Project::where(['is_deleted'=>0,'active_status'=>1,'category'=>'Upcoming'])->get();
      $data['title'] = 'upcoming';
      return view('frontend.upcoming',$data);
    }//end of method
    ...
    {
      $data['project'] = Project::where(['is_deleted'=>0,'active_status'=>1,'id'=>$request->project])->first();
      $data['ProductVariant'] = ProductVariant::where(['is_deleted'=>0,'active_status'=>1,'prod_id'=>$request->project])->get();
    //   dd($data['ProductVariant']);
    ...
    public function save_career(Request $request){
        ...
        if($validator->passes()) {
            ...
            if(!empty($request->file('image'))){
                ...
                $filedata['path'] = $formdata['subject'] = url('uploads').'/'.uniqid().'-'.$img->getClientOriginalName(); 
                $request->file('image')->move(public_path('uploads'), $formdata['subject']);
            }
            $row = Contact::insertGetId($formdata);
            $msg = 'Message Placed';
                 if($row){
